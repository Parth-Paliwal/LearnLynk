                        ...
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Button variant="outline" size="sm" className="flex items-center gap-2 p-5">
                                    ...
                                    <span className="text-[18] font-bold">Sort By</span>
                                </Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent align="end" className="w-[200px] ">
                                <DropdownMenuRadioGroup  value={sort} onValueChange={(value)=>setSort(value)}>
                                    {
                                        sortOptions.map((sortItems)=><DropdownMenuRadioItem value={sortItems.id} key={sortItems.id}>
                                                {sortItems.label}
                            ...
                            <Card onClick={()=>{navigate(`/course/details/${courseItem?._id}`)}} className="cursor-pointer border-2 border-gray-500" key={courseItem?._id}>
                                <CardContent className ="flex gap-4 p-4">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-[16px] text-gray-700 mb-2 mt-2">
                                            {
                                                `${courseItem?.curriculum?.length} ${courseItem?.curriculum?.length == 1 ? "Lecture" : "Lectures"} - ${courseItem?.level.charAt(0).toUpperCase() + courseItem?.level.slice(1)} Level`
                                            }
                                        </p>
